...
def initialize_classification_model(model_name, num_classes, pretrained = True, resume_from = None):
    ...
    if model_name == "resnet":
      ...
      input_size = 224
        ...
        model_ft = nn.Sequential(*list(vgg.children())[:-1])
        input_size = 224
...
if __name__ == '__main__':
    ...
    for index,row in zip(range(1),range(1)):#scheduler.iterrows():
      ...
      model_name = "vgg" #row["Model Name"]
      batch_size = 8 #row["Batch Size"]
      num_epochs = 5 #row["Num Epochs"]
      save_file = "init_network"#row["Network Name"]
      ...
      classification_feature_size = 512#classification_model
    ...
    previous_outputs = pd.read_excel(outputs + ".xls")
